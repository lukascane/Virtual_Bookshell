...
export default function EditProfileForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axiosInstance.patch(`http://localhost:3001/api/user/update`, data);
  ...
  return <div id='login' className='d-flex justify-content-center row align-items-center'>
    ...
    <input type="text" id="last_name_input" name="lastname" className="form-control w-100 mx-1 text-light mb-5 signUp border-bottom"/>
    <h4 className='mt-2'>Email:</h4>
    <input type="text" id="email_input" name="email"className="form-control w-100 mx-1 text-light mb-5 signUp border-bottom"/>
